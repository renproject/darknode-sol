...
export interface DarknodeSlasherContract {
    ...
    submitChallengeOrder(details: string, settlementID: number|string|BN, tokens: number|string|BN, price: number|string|BN, volume: number|string|BN, minimumVolume: number|string|BN, options?: Tx): Promise<TransactionObject<void>>;
    submitChallenge(_buyID: string, _sellID: string, options?: Tx): Promise<TransactionObject<void>>;
    address: string;
}
