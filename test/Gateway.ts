...
import {
    BTCGatewayInstance, GatewayInstance, GatewayRegistryInstance, renBTCInstance, RenERC20Instance,
...
contract("Gateway", ([owner, feeRecipient, user, malicious, proxyGovernanceAddress]) => {
    ...
    const mintTest = async (gateway: GatewayInstance, value: number | BN, n?: string) => {
    ...
    const burnTest = async (gateway: GatewayInstance, value: number | BN, btcAddress?: string, n?: string) => {
    ...
    describe("upgrading gateway", () => {
        let newGateway: GatewayInstance;

        it("can upgrade the gateway", async () => {
            newGateway = await BTCGateway.new(
                renbtc.address,
                feeRecipient,
                mintAuthority.address,
                mintFees,
    ...
    describe("gateway registry", () => {
        ...
        it("can update gateway for a token", async () => {
            ...
            const newBtcGateway = await BTCGateway.new(
                renbtc.address,
                feeRecipient,
                mintAuthority.address,
                mintFees,
