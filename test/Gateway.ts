    ...
    for (const rate of [
        ...
        describe(`Gateway with rate ${new BigNumber(rate).div(1e18).toFixed()}`, () => {
            ...
            const removeFee = async (token: RenERC20LogicV1Instance, amountUnderlying: number | BN, bips: number | BN) => {
                ...
                const amountAfterFeeUnderlying = await token.toUnderlying.call(amountAfterFee.toFixed(0, BigNumber.ROUND_DOWN));
            ...
            describe("can mint and burn", () => {
                const value = new BN(Math.floor(Math.random() * 20000) + 40000);
                it("can mint tokens with an unused hash, valid signature and pHash", async () => mintTest(btcGateway, value));
                it("can burn tokens", async () => burnTest(btcGateway, 24975));
            ...
            describe("recovering funds", () => {
                it("should be able to withdraw funds that are mistakenly sent to a Gateway", async () => {
                    let renbtcValue = new BN(Math.floor(Math.random() * 200000) + 10000);
                    [, , renbtcValue] = await mintTest(btcGateway, renbtcValue);
                    renbtcValue = new BN((await renbtc.fromUnderlying.call(renbtcValue)).toString());
                    ...
                    await burnTest(btcGateway, new BN((await renbtc.toUnderlying.call(balanceAfter)).toString()));
                });
