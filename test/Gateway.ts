...
import {
    GatewayLogicInstance, GatewayRegistryInstance, RenERC20Instance,
} from "../types/truffle-contracts";
import { log } from "./helper/logs";
import {
    deployProxy, ETHEREUM_TOKEN_ADDRESS, NULL, Ox, randomAddress, randomBytes,
...
contract("Gateway", ([owner, feeRecipient, user, malicious, proxyGovernanceAddress]) => {
    ...
    before(async () => {
        ...
        btcGateway = await deployProxy<GatewayLogicInstance>(web3, BTCGateway, GatewayLogic, proxyGovernanceAddress, [{ type: "address", value: renbtc.address }, { type: "address", value: feeRecipient }, { type: "address", value: mintAuthority.address }, { type: "uint16", value: mintFees.toString() }, { type: "uint16", value: burnFees.toString() }, { type: "uint256", value: 10000 }], { from: owner });
    ...
    const getFeeScaled = async (token: RenERC20Instance, amountUnderlying: number | BN, bips: number | BN) => {
        const amountScaled = new BN(await token.fromUnderlying.call(amountUnderlying));
        const amountFee = amountScaled.mul(new BN(bips)).div(new BN(10000));
        return amountFee;
    };

    const removeFee = async (token: RenERC20Instance, amountUnderlying: number | BN, bips: number | BN) => {
        const amount = new BN(await token.fromUnderlying.call(amountUnderlying));
        const amountFee = amount.mul(new BN(bips)).div(new BN(10000));
        const amountAfterFee = amount.sub(amountFee);
        const amountAfterFeeUnderlying = await token.toUnderlying.call(amountAfterFee);
        const amountAfterFeeRepresentable = await token.fromUnderlying.call(amountAfterFeeUnderlying);
        // The following may slightly vary from amountAfterFeeUnderlying.
        const amountAfterFeeRepresentableUnderlying = new BN(await token.toUnderlying.call(amountAfterFeeRepresentable));

        return amountAfterFeeRepresentableUnderlying;
    };

    const mintTest = async (gateway: GatewayLogicInstance, value: number | BN, n?: string) => {
        const nHash = randomBytes(32);
        const pHash = randomBytes(32);

        const hash = await gateway.hashForSignature.call(pHash, value, user, nHash);
        ...
        const hashForSignature = await gateway.hashForSignature.call(pHash, value, user, nHash);
        (await gateway.verifySignature.call(hashForSignature, sigString))
            .should.be.true;
        ...
        (await gateway.mint(pHash, value, nHash, sigString, { from: user }) as any)
            .should.emit.logs([
                log(
                    ...
                    {
                        _to: user,
                        _amount: await removeFee(renbtc, value, mintFees),
                        _n: n !== undefined ? n : _n,
                    },
                ),
            ]);
    ...
    const burnTest = async (gateway: GatewayLogicInstance, value: number | BN, btcAddress?: string, n?: string) => {
        // Note: we don't use `||` because we want to pass in `""`
        btcAddress = btcAddress !== undefined ? btcAddress : randomBytes(35);

        const balanceBefore = new BN((await renbtc.balanceOf.call(user)).toString());
        const _n = await gateway.nextN.call();

        const tokenAddress = await gateway.token.call();
        const token = await RenERC20.at(tokenAddress);
        ...
        (await gateway.burn(btcAddress, await token.fromUnderlying.call(value), { from: user }) as any)
            .should.emit.logs([
                log(
                    "LogBurn",
                    {
    ...
    describe("can mint and burn", () => {
        ...
        it("can mint tokens with an unused hash, valid signature and pHash", async () => mintTest(btcGateway, value));
        it("can burn tokens", async () => burnTest(btcGateway, await removeFee(renbtc, value, mintFees)));
        it("won't mint for the same nHash and pHash twice", async () => {
            const [pHash, nHash] = await mintTest(btcGateway, value);

            const hash = await btcGateway.hashForSignature.call(pHash, value.toNumber(), user, nHash);
            const sig = ecsign(Buffer.from(hash.slice(2), "hex"), privKey);
            const sigString = Ox(`${sig.r.toString("hex")}${sig.s.toString("hex")}${(sig.v).toString(16)}`);

            await btcGateway.mint(pHash, value.toNumber(), nHash, sigString, { from: user })
                .should.be.rejectedWith(/Gateway: nonce hash already spent/);
        });
        ...
        it("won't mint with an invalid signature", async () => {
            ...
            const hash = await btcGateway.hashForSignature.call(pHash, value.toNumber(), user, nHash1);
            const sig = ecsign(Buffer.from(hash.slice(2), "hex"), privKey);

            const sigString = Ox(`${sig.r.toString("hex")}${sig.s.toString("hex")}${(sig.v).toString(16)}`);

            await btcGateway.mint(pHash, value.toNumber(), nHash2, sigString, { from: user })
                .should.be.rejectedWith(/Gateway: invalid signature/);
        });
        ...
        it("can't burn less than minimum burn amount", async () => {
            await burnTest(btcGateway, 5000)
                .should.be.rejectedWith(/Gateway: amount is less than the minimum burn amount/);
        });
        ...
        it("won't mint for a signature's complement", async () => {
            ...
            await btcGateway.mint(pHash, value.toNumber(), nHash, altSigString, { from: user })
    ...
    describe("updating fee recipient and mint authority", () => {
        it("can upgrade fee recipient", async () => {
            await (btcGateway.updateFeeRecipient(malicious, { from: malicious }))
                .should.be.rejectedWith(/Ownable: caller is not the owner/);
            await (btcGateway.updateFeeRecipient(NULL, { from: owner }))
                .should.be.rejectedWith(/Gateway: fee recipient cannot be 0x0/);
            await btcGateway.updateFeeRecipient(user, { from: owner });
            await btcGateway.updateFeeRecipient(feeRecipient, { from: owner });
        });

        it("can upgrade mint fee", async () => {
            const currentFee = await btcGateway.mintFee.call();
            await (btcGateway.updateMintFee(0, { from: malicious }))
                .should.be.rejectedWith(/Ownable: caller is not the owner/);
            await btcGateway.updateMintFee(0, { from: owner });
            await btcGateway.updateMintFee(currentFee, { from: owner });
        });

        it("can upgrade burn fee", async () => {
            const currentFee = await btcGateway.burnFee.call();
            await (btcGateway.updateBurnFee(0, { from: malicious }))
                .should.be.rejectedWith(/Ownable: caller is not the owner/);
            await btcGateway.updateBurnFee(0, { from: owner });
            await btcGateway.updateBurnFee(currentFee, { from: owner });
        });
        ...
        it("cannot upgrade mint authority to zero address", async () => {
            await (btcGateway.updateMintAuthority(NULL, { from: owner }))
                .should.be.rejectedWith(/Gateway: mintAuthority cannot be set to address zero/);
        });

        it("can upgrade min burn amount", async () => {
            await (btcGateway.updateMinimumBurnAmount(malicious, { from: malicious }))
                .should.be.rejectedWith(/Ownable: caller is not the owner/);
            await btcGateway.updateMinimumBurnAmount(8000, { from: owner });
            await btcGateway.updateMinimumBurnAmount(10000, { from: owner });
        });
    });

    describe("upgrading gateway", () => {
        let newGateway: GatewayLogicInstance;

        it("can upgrade the gateway", async () => {
            newGateway = await deployProxy<GatewayLogicInstance>(web3, BTCGateway, GatewayLogic, proxyGovernanceAddress, [{ type: "address", value: renbtc.address }, { type: "address", value: feeRecipient }, { type: "address", value: mintAuthority.address }, { type: "uint16", value: mintFees.toString() }, { type: "uint16", value: burnFees.toString() }, { type: "uint256", value: 10000 }], { from: owner });
            ...
            await (btcGateway.transferTokenOwnership(newGateway.address, { from: malicious }))
                .should.be.rejectedWith(/Ownable: caller is not the owner/);

            await btcGateway.transferTokenOwnership(newGateway.address, { from: owner });
            (await renbtc.owner.call()).should.equal(newGateway.address);
        });
        ...
        it("can't upgrade to an invalid gateway", async () => {
            await (newGateway.transferTokenOwnership(malicious, { from: owner }))
                .should.be.rejectedWith(/revert/); // Tries to call ".claim" on non-contract address

            await renbtc.claimOwnership({ from: malicious })
        ...
        it("can reset the upgrade", async () => {
            ...
            await (btcGateway.transferTokenOwnership(NULL, { from: owner }))
                .should.be.rejectedWith(/Ownable: caller is not the owner/);

            // Upgrade newGateway to point to btcGateway
            await newGateway.transferTokenOwnership(btcGateway.address, { from: owner });

            (await renbtc.owner.call()).should.equal(btcGateway.address);
        });
    });
    ...
    describe("recovering funds", () => {
        it("should be able to withdraw funds that are mistakenly sent to a Gateway", async () => {
            ...
            renbtcValue = await removeFee(renbtc, renbtcValue, mintFees);
            await renbtc.transfer(btcGateway.address, renbtcValue, { from: user });
            ...
            await btcGateway.recoverTokens(ren.address, { from: malicious })
                .should.be.rejectedWith(/Ownable: caller is not the owner/);

            // Recover renBTC
            const balanceBefore = new BN(await renbtc.balanceOf.call(owner));
            await btcGateway.recoverTokens(renbtc.address, { from: owner });
            ...
            await btcGateway.recoverTokens(ren.address, { from: owner });
            ...
            await forceSend.send(btcGateway.address, { value: "1" });
        ...
        it("should be able to withdraw funds that are mistakenly sent to a token", async () => {
            ...
            // Only the owner can recover tokens
            await renbtc.recoverTokens(ren.address, { from: malicious })
                .should.be.rejectedWith(/Ownable: caller is not the owner/);

            const claimer = await Claimer.new(renbtc.address);
            await (btcGateway.transferTokenOwnership(claimer.address, { from: owner }));
            await claimer.transferTokenOwnership(owner, { from: owner });
            await renbtc.claimOwnership({ from: owner });

            // Recover REN
            const initialRenBalance = new BN((await ren.balanceOf.call(owner)).toString());
            await renbtc.recoverTokens(ren.address, { from: owner });
            const finalRenBalance = new BN((await ren.balanceOf.call(owner)).toString());
            finalRenBalance.sub(initialRenBalance).should.bignumber.equal(renAmount);
    ...
    describe("gateway registry", () => {
        ...
        it("symbol validation", async () => {
            (await registry.symbolIsValid.call("BTC"))
                .should.be.true;
            (await registry.symbolIsValid.call("ZEC"))
                .should.be.true;
            (await registry.symbolIsValid.call("BCH"))
                .should.be.true;

            (await registry.symbolIsValid.call("zеc!"))
                .should.be.false;
        ...
        it("can register gateways", async () => {
            ...
            await registry.setGateway("BTC", renbtc.address, btcGateway.address)
                .should.be.rejectedWith(/GatewayRegistry: gateway already registered/);
            await registry.setGateway("BTC", renbtc.address, NULL)
                .should.be.rejectedWith(/GatewayRegistry: token already registered/);

            await registry.setGateway("BTC!", NULL, NULL)
                .should.be.rejectedWith(/GatewayRegistry: symbol must be alphanumeric/);
        ...
        it("can retrieve gateways", async () => {
            { // Try to register token with an existing symbol
                ...
                await registry.setGateway("BTC", altRenBTC.address, NULL)
                    .should.be.rejectedWith(/GatewayRegistry: symbol already registered/);
            }

            (await registry.getGatewayByToken.call(renbtc.address))
            ...
            { // The first 10 gateways starting from NULL
                const gateway = await registry.getGateways.call(NULL, 10);
                gateway[0].should.equal(btcGateway.address);
                gateway[1].should.equal(NULL);
                gateway.length.should.equal(10);

                const renTokens = await registry.getRenTokens.call(NULL, 10);
            ...
            { // Get all the gateways starting from NULL
                const gateway = await registry.getGateways.call(NULL, 0);
                gateway[0].should.equal(btcGateway.address);
                gateway.length.should.equal(1);

                const renTokens = await registry.getRenTokens.call(NULL, 0);
                renTokens[0].should.equal(renbtc.address);
                renTokens.length.should.equal(1);
            }

            { // Starting from first entry
                const gateway = await registry.getGateways.call(btcGateway.address, 10);
                gateway[0].should.equal(btcGateway.address);
                gateway[1].should.equal(NULL);
                gateway.length.should.equal(10);

                const renTokens = await registry.getRenTokens.call(renbtc.address, 10);
                renTokens[0].should.equal(renbtc.address);
                renTokens[1].should.equal(NULL);
                renTokens.length.should.equal(10);
            }
            ...
            { // Get all the gateways starting from first entry
                const gateways = await registry.getGateways.call(btcGateway.address, 0);
                gateways[0].should.equal(btcGateway.address);
                gateways.length.should.equal(1);

                const renTokens = await registry.getRenTokens.call(renbtc.address, 0);
                renTokens[0].should.equal(renbtc.address);
                renTokens.length.should.equal(1);
            }
        });
        ...
        it("can update gateway for a token", async () => {
            (await registry.getGatewayByToken.call(renbtc.address)).should.equal(btcGateway.address);

            const newBtcGateway = await deployProxy<GatewayLogicInstance>(web3, BTCGateway, GatewayLogic, proxyGovernanceAddress, [{ type: "address", value: renbtc.address }, { type: "address", value: feeRecipient }, { type: "address", value: mintAuthority.address }, { type: "uint16", value: mintFees.toString() }, { type: "uint16", value: burnFees.toString() }, { type: "uint256", value: 10000 }], { from: owner });

            await registry.updateGateway(renbtc.address, newBtcGateway.address);

            (await registry.getGatewayByToken.call(renbtc.address)).should.equal(newBtcGateway.address);
        });

        it("can't update gateway for an unregistered token", async () => {
            await registry.updateGateway(ETHEREUM_TOKEN_ADDRESS, randomAddress())
                .should.be.rejectedWith(/GatewayRegistry: token not registered/);
        });
        ...
        it("can deregister gateways", async () => {
            ...
            await registry.removeGateway("BTC")
                .should.be.rejectedWith(/GatewayRegistry: symbol not registered/);
        });
