...
contract("Validate", (accounts: string[]) => {
    ...
    describe("when generating messages", async () => {
        ...
        it("should correctly generate secret messages", async () => {
            ...
            const rawMsg = await validateTest.secretMessage.call(a, b, c, d, e, f);
        ...
        it("should correctly generate the propose message", async () => {
            ...
            const rawMsg = await validateTest.proposeMessage.call(height, round, hexBlockhash, validRound);
        ...
        it("should correctly generate the prevote message", async () => {
            ...
            const rawMsg = await validateTest.prevoteMessage.call(height, round, hexBlockhash);
        ...
        it("should correctly generate the precommit message", async () => {
            ...
            const rawMsg = await validateTest.precommitMessage.call(height, round, hexBlockhash);
    ...
    describe("when recovering signatures", async () => {
        ...
        it("can recover the signer of a propose message", async () => {
            ...
            const signer = await validateTest.recoverPropose.call(height, round, hexBlockhash, validRound, sigString);
        ...
        it("can recover the signer of a prevote message", async () => {
            ...
            const sig = ecsign(Buffer.from(hash, "hex"), darknode.privateKey);
            const sigString = Ox(`${sig.r.toString("hex")}${sig.s.toString("hex")}${(sig.v).toString(16)}`);
            const signer = await validateTest.recoverPrevote.call(height, round, hexBlockhash, sigString);
            signer.should.equal(darknode.account.address);
        });
        ...
        it("can recover the signer of a precommit message", async () => {
            ...
            const sig = ecsign(Buffer.from(hash, "hex"), darknode.privateKey);
            const sigString = Ox(`${sig.r.toString("hex")}${sig.s.toString("hex")}${(sig.v).toString(16)}`);
            const signer = await validateTest.recoverPrecommit.call(height, round, hexBlockhash, sigString);
            signer.should.equal(darknode.account.address);
        });
        ...
        it("can recover the signer of a secret message", async () => {
            ...
            const sig = ecsign(Buffer.from(hash, "hex"), darknode.privateKey);
            const sigString = Ox(`${sig.r.toString("hex")}${sig.s.toString("hex")}${(sig.v).toString(16)}`);
            const signer = await validateTest.recoverSecret.call(a, b, c, d, e, f, sigString);
            signer.should.equal(darknode.account.address);
        });
