...
const signPermit = async (token: RenERC20Instance, privateKey: Buffer, from: string, to: string, allowed: boolean) => {
    ...
    const digest = keccak256(solidityPack(
    ...
    toChecksumAddress(Ox(pubToAddress(ecrecover(digest, v, r, s))))
        .should.equal(from);
...
contract("ERC20WithPermit", ([owner, secondUser, malicious]) => {
    ...
    before(async () => {
        ...
        token = await deployProxy<RenERC20LogicV1Instance>(web3, RenBTC, RenERC20LogicV1, proxyAdmin.address, [
            { name: "_chainId", type: "uint256", value: await web3.eth.net.getId() },
            { name: "_nextOwner", type: "address", value: owner },
            { name: "_initialRate", type: "uint256", value: "500000000000000000" },
    ...
    it("approve with permit", async () => {
        ...
        // Approve and deposit
        (await submitPermit(token, privateKey, firstUser, secondUser, true) as any)
            .should.emit.logs([
                log(
                    "Approval",
                    {
        ...
        await token.transferFrom(firstUser, secondUser, value, { from: secondUser });

        // Compare balances after depositing
        (await token.balanceOf.call(firstUser))
            .should.bignumber.equal(firstBefore.sub(new BN(value)));
        (await token.balanceOf.call(secondUser))
            .should.bignumber.equal(secondBefore.add(new BN(value)));
    });
    ...
    it("revoke approval", async () => {
        ...
        (await submitPermit(token, privateKey, firstUser, secondUser, false) as any)
            .should.emit.logs([
                log(
                    "Approval",
                    {
    ...
    it("can't resubmit signature", async () => {
        // Approve and deposit
        const { r, s, v, nonce, expiry } = (await signPermit(token, privateKey, firstUser, malicious, true));
        await token.permit(firstUser, malicious, nonce, expiry, true, v, Ox(r), Ox(s), { from: malicious });
        await submitPermit(token, privateKey, firstUser, malicious, false);

        await token.permit(firstUser, malicious, nonce, expiry, true, v, Ox(r), Ox(s), { from: malicious })
            .should.be.rejectedWith(/ERC20WithRate: invalid nonce/);

        await token.permit(firstUser, malicious, (await token.nonces.call(firstUser)), expiry, true, v, Ox(r), Ox(s), { from: malicious })
            .should.be.rejectedWith(/ERC20WithRate: invalid signature/);
    });

    it("can't change address", async () => {
        // Approve and deposit
        const { r, s, v, nonce, expiry } = (await signPermit(token, privateKey, firstUser, secondUser, true));
        await token.permit(firstUser, malicious, nonce, expiry, true, v, Ox(r), Ox(s), { from: malicious })
            .should.be.rejectedWith(/ERC20WithRate: invalid signature/);
    });
    ...
    it("can't submit permit for 0x0", async () => {
        ...
        await token.permit(NULL, malicious, nonce, expiry, true, 27, NULL32, NULL32, { from: malicious })
