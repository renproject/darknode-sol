...
contract("LinkedList", () => {
    ...
    it("can prepend", async () => {
        ...
        (await linkedList.previous(NODE1)).should.equal(NODE2);
    });
    ...
    it("should not insert after a node not in the list", async () => {
        await linkedList
            .insertAfter(NOT_NODE1, NOT_NODE2)
            .should.be.rejectedWith(/LinkedList: not in list/);
    });

    it("should not insert before a node not in the list", async () => {
        await linkedList
            .insertBefore(NOT_NODE1, NOT_NODE2)
            .should.be.rejectedWith(/LinkedList: not in list/);
    });
    ...
    it("should return elements in the list", async () => {
        let gateways = await linkedList.elements(NODE1, 1);
        gateways[0].should.equal(NODE1);
        gateways.length.should.equal(1);

        gateways = await linkedList.elements(NODE2, 2);
        ...
        gateways = await linkedList.elements(NODE1, 10);
        gateways[0].should.equal(NODE1);
