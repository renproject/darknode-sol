...
contract SwapERC20 {
  ...
  enum SwapStatus {
    pending, initiated, redeemed
  }
  
  mapping (bytes32 => Swap) public swaps;
  mapping (bytes32 => SwapStatus) public swapStatus;
    
  function initiate(bytes32 _orderID, address _owner, bytes _signature, uint256 _value, address _tokenAddress) public {
    require(swapStatus[_orderID] == SwapStatus.pending);
    require(verifySignature(_owner, _orderID, _signature));
    Token t = Token(_tokenAddress);
    require(t.allowance(address(this), msg.sender) >= _value);
    require(t.transferFrom(address(this), msg.sender, _value));
    swaps[_orderID].sig = _signature;
    swaps[_orderID].owner = _owner;
    swaps[_orderID].tokenAddress = _tokenAddress;
    swaps[_orderID].value = _value;
    swapStatus[_orderID] = SwapStatus.initiated;
  }

  function audit(bytes32 _orderID) public view returns(address, uint256) {
    return (swaps[_orderID].tokenAddress, swaps[_orderID].value);
  }
  ...
  function redeem(bytes32 _orderIDA, bytes32 _orderIDB, bytes _signatureB, bytes _matchSigA, bytes _matchSigB, bytes _receiverSig) public {
    ...
    Signature memory sigB = decodeSignature(_signatureB);
    address ownerB = ecrecover(_orderIDB, sigB.v, sigB.r, sigB.s);
    ...
    Token t = Token(swaps[_orderIDA].tokenAddress);
    require(t.transfer(msg.sender, swaps[_orderIDA].value));
  }
  ...
  function getMatchID(bytes _sigA, bytes _sigB) internal pure returns (bytes32) {
  ...
  function verifySignature(address _signer, bytes32 _hash, bytes _signature) internal pure returns (bool) {
    return (_signer == ecrecover(_hash,uint8(_signature[0]),toBytes32(_signature, 1),toBytes32(_signature, 33)));
  }

  function decodeSignature(bytes signature) internal pure returns (Signature) {
    return Signature({
      v: uint8(signature[0]),
